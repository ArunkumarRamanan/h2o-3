...
public class FVecParseWriter extends Iced implements StreamParseWriter {
  ...
  public FVecParseWriter(Vec.VectorGroup vg, int cidx, Categorical[] categoricals, byte[] ctypes, int chunkSize,
    ...
    boolean ctypesShrunk = avs.length < categoricals.length; // may not be the same if user sets skipped_columns already.
         // Required not-null
    if ((parse_columns_indices!=null) && (parse_columns_indices.length == categoricals.length)) { // for nextChunk() calls in gzip/zip parser
      _ctypes = ctypes;
      ...
      if (parse_columns_indices == null) {
        ...
        _ctypes = ctypes == null ? null : new byte[parseColNum];
        _categoricals = categoricals == null ? null : new Categorical[parseColNum];
